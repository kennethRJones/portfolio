	...
	while (command != "Quit")
	{
		if(command == "Initialize")
		{
			...
			try {
				...
				outFile << command << ": " <<date.GetMonthAsString() << " " <<
				date.GetDay() << ", " << date.GetYear() << endl;
			}
			catch (string msg)
			{
				outFile << msg << endl;
		...
		else if (command == "ComparedTo")
		{
			...
			{
			case LESS	: outFile << date.GetMonth() << " " <<
							  date.GetDay() << ", " << date.GetYear();
							outFile<< " comes before ";
							outFile << date2.GetMonth() << " " <<
								date2.GetDay() << ", "
								<< date2.GetYear() << endl;
							break;
			case GREATER:	outFile << date2.GetMonth() << " " <<
							date2.GetDay() << ", " << date2.GetYear();
							outFile << " comes before ";
							outFile << date.GetMonth() << " " <<
							date.GetDay() << ", " << date.GetYear() << endl;
							break;
			case EQUAL: outFile << date.GetMonth() << " " <<
						date.GetDay() << ", " << date.GetYear();
						outFile << " and ";
						outFile << date2.GetMonth() << " " <<
						date2.GetDay() << ", " << date2.GetYear() << endl;
